                ...
                <?php foreach ($select_prescription_by_id as $key => $value) { ?>
                    ...
                    <div class="form-group">
                             <label class="col-sm-12"><?php echo get_phrase('doctor');?>*</label>
                                 <div class="col-sm-12">
									<select class="select2 form-control" id="doctor_id" name="doctor_id" onchange="return get_doctor_patient_edit(this.value, <?php echo $prescription_id;?>)" required>
                                         	<option data-tokens=""><?php echo get_phrase('select_doctor');?></option>
                                    	<?php 
										$doctor = $this->db->get('doctor')->result_array();
										foreach($doctor as $row):
										?>
                                    		<option value="<?php echo $row['doctor_id'];?>"<?php if($value['doctor_id'] == $row['doctor_id']) echo 'selected="selected"';?>><?php echo $row['name'];?></option>
									 <?php endforeach;?>
                                    </select>
								</div>
					</div>
